        ...
        <div class="flex flex-col lg:mx-32">
            ...
            <div class="isolate overflow-hidden bg-white px-6 py-20 lg:overflow-visible lg:px-0">
                <div class="mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 lg:mx-0 lg:max-w-none lg:grid-cols-1 lg:items-start lg:gap-y-10">
                    ...
                    <div class="mb-12 space-y-6 text-left text-lg text-gray-800">
                        <div class="flex items-center space-x-4">
                            @if (!$usuario->suscripcion == 'NULL')
                                <div class="p-4 mb-4 text-sm text-white rounded-xl bg-indigo-600  font-normal" role="alert">
                                    <span class="font-semibold mr-2">No estás suscripto a ningún plan actualmente.</span> Suscribite <a href="{{ route('suscripciones.index', auth()->user()->user_id) }}" class="font-bold ml-1"> acá</a>.
                                </div>
                            @else
                            ...
                            <div class="mt-8 gap-x-6 gap-y-8">
                                <div class="flex items-center space-x-4">
                                    <svg class="flex-shrink-0 w-6 h-6 text-red-600" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <p>Estás suscripto al plan : <span class="font-bold">{{ $usuario->suscripcion->plan }}</span>.<br><span class="text-xs">Fecha de suscripción: {{ ($usuario->updated_at)->format('d F Y') }}.</span>
                                        </p>
                                </div>
                            </div>
                            <div class="mt-8 gap-x-6 gap-y-8">
