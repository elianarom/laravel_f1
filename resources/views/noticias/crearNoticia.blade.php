        ...
        <div class="flex flex-col lg:mx-32">
            <div class="isolate overflow-hidden bg-white px-6 py-20 lg:overflow-visible lg:px-0">
                <div
                    ...
                    <h1 class="mb-3 font-semibold text-2xl">Publicar noticia</h1>
                    @if ($errors->any())
                        <div class="p-2 mb-2 text-base font-bold text-red-600 rounded-lg bg-red-50">* Hay errores en los datos del formulario.</div>
                    @endif
                    <form action="{{ route('noticias.crearProceso') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="space-y-12">
                            <div class="border-b border-gray-900/10 pb-12">
                                ...
                                <div class="mt-2 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                        ...
                                        <div class="mt-2">
                                            <div
                                                ...
                                                <input type="text" name="titulo" id="titulo"
                                                    ...
                                                    placeholder="Título de la noticia" value="{{ old('titulo') }}">
                                            </div>
                                            @error('titulo')
                                    ...
                                    <div class="col-span-full">
                                        ...
                                        <div class="mt-2">
                                            <textarea id="descripcion" name="descripcion" rows="3"
                                                class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-red-600 sm:text-sm sm:leading-6">{{ old('descripcion') }}</textarea>
                                            @error('descripcion')
                                                <div class="mt-2 font-semibold text-xs text-red-600">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="col-span-full">
                                        <label for="portada"
                                            class="block text-sm font-medium leading-6 text-gray-900">Foto de portada de
                                            la noticia</label>
                                        <div
                                    ...
                                    <div class="sm:col-span-4">
                                        <label for="portada_descripcion"
                                            class="block text-sm font-medium leading-6 text-gray-900">Descripción de la
                                            portada</label>
                                        <div class="mt-2">
                                            <div
                                                class="flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-red-600 sm:max-w-md">
                                        ...
                                        <div class="mt-2">
                                            <div
                                                ...
                                                <input type="date" name="fecha_publicacion" id="fecha_publicacion"
                                                    class="block flex-1 border-0 bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6"
                                                    placeholder="Fecha de publicacion" value="{{ old('fecha_publicacion') }}">
                                            </div>
                                            ...
                                            <select id="escuderia_fk" name="escuderia_fk"
                                                ...
                                                @foreach ($escuderias as $escuderia)
                                                    <option value="{{ $escuderia->escuderia_id }}"
                                                        @selected($escuderia->escuderia_id == old('escuderia_fk'))>
                                                        {{ $escuderia->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="mt-6 space-y-6">
                                        <div class="relative flex gap-x-3">
                                            <div class="text-sm leading-6">
                                                ...
                                                <label for="comments" class="flex h-6 items-center">
                                                    <input type="checkbox" name="categoria_fks[]" class="h-4 w-4 rounded border-gray-300 text-red-600 focus:ring-red-600"
                                                    value="{{ $categoria->categoria_id }}"
                                                        @checked(in_array($categoria->categoria_id, old('categoria_fks', [])))>{{ $categoria->nombre }}
                                                </label>
                                                @endforeach
                                            </div>
                                        </div>
                        ...
                        <div class="mt-20 flex items-center justify-start gap-x-6">
                            <button type="submit"
                                class="rounded-md bg-black px-3 py-2 text-sm font-semibold text-white shadow-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2">Publicar</button>
                        </div>
                    </form>
                </div>
            </div>
